...
struct f4_data f4_dif(struct f4_data, struct f4_data);



int f4_read(struct f4_data *t) {
    return (scanf("%d:%d:%d", &t->field_A, &t->field_B, &t->field_C) == 3);
}

void f4_write(struct f4_data t) {
    if (t.field_A > 0) {
        printf("%2d:%02d:%02d\n", t.field_A, t.field_B, t.field_C);
        return;
    }
    if (t.field_B > 0) {
        printf("%5d:%02d\n", t.field_B, t.field_C);
...
struct f4_data f4_diff(struct f4_data par1, struct f4_data par2) {
    par1.field_C = ((par2.field_A-par1.field_A)*60+(par2.field_B-par1.field_B))*60+(par2.field_C-par1.field_C);
    par1.field_A = par1.field_C / 3600.0;
    par1.field_C /= 3600.0;
    par1.field_B = par1.field_C / 60.0;
    par1.field_C /= 60.0;
    return par1;
}
...
#if !defined(COMPILEONBIRO) || defined(COMPILEFOR3)
  int eredmeny, i;
  struct {char str[128]; int eredmeny;} testlist[1] = {{"720739", 4}};
  for (i = 0; i < 1; ++i) {
