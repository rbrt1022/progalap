...
void strmegforditas(char str[]) {
    ...
    int elemszamlalo;
    char szavak[] = "";
    //String elemeinek számának meghatározása
    /*for (elemszamlalo = 0; str[elemszamlalo] = '\0'; elemszamlalo++)
    {
        printf("%d",elemszamlalo);
    }*/

    while (str[elemszamlalo] != '\0')
    {
        elemszamlalo++;
        //printf("Elemszamlalo = %d", elemszamlalo);
    }
    ...
    //Elemek csereberéje
    for (int i = 0, j = elemszamlalo; i = j; i++, j--)
    {
        szavak[j] = str[i];
        //szavak[i] = str[j];
        //str[i] = cser2;
        //str[j] = cser1;
        printf("%c és %c cserének helyet\n", szavak[j], str[i]);
    }
